...
public Action:ShowTeamStatsToPlayer(iClient, args)
{
	...
	if(probe == true)
	{
		...
		{
			if(IsClientInGame(i) && IsFakeClient(i) == false && g_iClientTeam[i] == 2)
			{
	...
	if(probe == true)
	{
		...
		for(new i = 1; i<MaxClients; i++)
		{
			if(IsClientInGame(i) && IsFakeClient(i) == false && g_iClientTeam[i] == 3)
			{
	...
	if(probe == true)
	{
		...
		for(new i = 1; i<MaxClients; i++)
		{
			if(IsClientInGame(i) && IsFakeClient(i) == false && g_iClientTeam[i] == 1)
			{
