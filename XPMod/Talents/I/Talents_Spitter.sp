...
{
	if(g_bTalentsConfirmed[iClient] == false || g_iPuppetLevel[iClient] <= 0)
		return;
...
void ConjureFromBagOfSpits(int iClient, float xyzLocation[3])
{
	...
	{
		case BAG_OF_SPITS_MINI_ARMY: 		SpawnCIAroundLocation(xyzLocation, 12, UNCOMMON_CI_NONE, CI_REALLY_SMALL, ENHANCED_CI_TYPE_RANDOM);
		case BAG_OF_SPITS_MUSCLE_CREW: 		SpawnCIAroundLocation(xyzLocation, 6, UNCOMMON_CI_NONE, CI_REALLY_BIG, ENHANCED_CI_TYPE_NONE);
		case BAG_OF_SPITS_ENHANCED_JIMMY: 	SpawnCIAroundLocation(xyzLocation, 1, UNCOMMON_CI_JIMMY, CI_REALLY_BIG_JIMMY, ENHANCED_CI_TYPE_RANDOM);
		case BAG_OF_SPITS_NECROFEASTER: 	SpawnCIAroundLocation(xyzLocation, 10, UNCOMMON_CI_CEDA, CI_BIG, ENHANCED_CI_TYPE_NECRO);	// Add life stealing also, when code supports it
		case BAG_OF_SPITS_KILLER_KLOWNS: 	SpawnCIAroundLocation(xyzLocation, 5, UNCOMMON_CI_CLOWN, CI_REALLY_BIG, ENHANCED_CI_TYPE_VAMPIRIC);
	}
